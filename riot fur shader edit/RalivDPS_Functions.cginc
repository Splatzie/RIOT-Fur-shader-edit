...
void PenetratorReshape(inout float4 vertex, inout float4 normal) {
	...
	normal.xyz = normalize( ( ( -bezierSpaceX * normal.x ) + ( bezierSpaceY * normal.y ) + ( bezierDerivitive * normal.z )) );
	...
	void OrificeReshape(inout float4 vertex, inout float4 normal, float4 tangent, int vertexId) {
		float orificeChannel=0;
